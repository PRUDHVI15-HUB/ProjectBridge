  ...
  return (
    <section className="py-20 md:py-28 bg-white border-t border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 5 Column Grid — staggered reveal */}
        <div ref={gridRef} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
          {items.map((item, index) => {
            const Icon = item.icon;
            return (
              <div
                key={index}
                className="pb-reveal group bg-white border border-slate-200/80 rounded-xl p-6 hover:border-slate-300 hover:shadow-sm transition card-hover flex flex-col justify-start text-left"
              >
                <div className="flex items-center gap-2 mb-3">
                  <Icon className="w-4 h-4 text-indigo-600 flex-shrink-0 transition-transform duration-200 group-hover:scale-110 group-hover:text-indigo-700" />
